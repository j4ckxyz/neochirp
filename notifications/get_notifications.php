...
// Get grouped notifications: group by (type, chirp_id), most recent first
$stmt = $db->prepare("
    SELECT
        n.type,
        n.chirp_id,
...
$results = [];
foreach ($groups as $g) {
    // Get up to 2 most recent actors for this group
    $aStmt = $db->prepare("
    ...
    // If type is 'reply', also fetch the reply chirp details
    $replyData = null;
    if ($g['type'] === 'reply') {
        $rStmt = $db->prepare("
